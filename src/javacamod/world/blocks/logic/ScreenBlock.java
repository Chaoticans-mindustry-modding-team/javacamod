...
public class ScreenBlock extends Block{
    ...
    public int[] palette = {0xff0000_ff, 0x770000_ff, 0x300000_ff, 0x00ff00_ff, 0x007700_ff, 0x003000_ff, 0x0000ff_ff, 0x000077_ff, 0x000030_ff, 0xffffff_ff, 0x898989_ff, 0x000000_ff, 0xffff00_ff, 0x777700_ff, 0x303000_ff, 0x00ffff_ff, 0x007777_ff, 0x003030_ff, 0xff00ff_ff, 0x770077_ff, 0x300030_ff, 0xff6600_ff, 0x774400_ff, 0x66ff00_ff, 0x447700_ff, 0x00ff66_ff, 0x007744_ff, 0x0066ff_ff, 0x004477_ff, 0x6600ff_ff, 0x440077_ff, 0xff0066_ff, 0x770044_ff};
    public int bitsPerPixel;
    public IntIntMap colorToIndex = new IntIntMap();

		public TextureRegion side1, side2, corner1, corner2;
    ...
    public ScreenBlock(String name){
        ...
        config(byte[].class, (ScreenBuild build, byte[] bytes) -> {
    ...
    public Pixmap makePixmap(byte[] data){
        if(previewPixmap == null){
            previewPixmap = new Pixmap(screenSize, screenSize);
        }
        ...
        for(int i = 0; i < pixels; i++){
            ...
            previewPixmap.set(i % screenSize, i / screenSize, palette[pal]);
        }
    ...
    public class ScreenBuild extends Building{
        public @Nullable Texture texture;
        public byte[] data = new byte[Mathf.ceil(screenSize * screenSize * bitsPerPixel / 8f)];
        public int blending;
        ...
        public byte[] packPixmap(Pixmap pixmap){
            ...
            int pixels = screenSize * screenSize;
            for(int i = 0; i < pixels; i++){
                int color = pixmap.get(i % screenSize, i / screenSize);
        ...
        public void buildConfiguration(Table table){
            table.button(Icon.pencil, Styles.cleari, () -> {
                ...
                dialog.cont.table(Tex.pane, body -> {
...

                        {
                            addListener(new InputListener(){
                                int convertX(float ex){
                                    return (int)((ex - x) / width * screenSize);
                                }

                                int convertY(float ey){
                                    return pix.height - 1 - (int)((ey - y) / height * screenSize);
                                }
                    ...
                    }, new GridImage(screenSize, screenSize){{
        ...
        public void write(Writes write){
            ...
            //for future screen resizing events
            write.i(data.length);
            write.b(data);
        }
